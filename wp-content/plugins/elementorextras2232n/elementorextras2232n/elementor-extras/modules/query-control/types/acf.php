...
class Acf extends Meta_Base {
	...
	public function get_autocomplete_values( array $data ) {
		...
		foreach ( $query->posts as $post ) {
			...
			$display_type 		= ( ! empty( $options['show_type'] ) && $options['show_type'] ) ? $this->get_title() : '';
			$display_field_type = ( ! empty( $options['show_field_type'] ) &&  $options['show_field_type'] ) ? $this->get_acf_field_type_label( $field_type ) : '';
			$display 			= ( ( ! empty( $options['show_type'] ) && $options['show_type'] ) || ( ! empty( $options['show_field_type'] ) && $options['show_field_type'] ) ) ? ': ' . $display : $display;
